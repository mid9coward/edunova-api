...
exports.CourseChatbotService = void 0;
...
class CourseChatbotService {
     ...
     */
    async handleOrderStatusQuery(message, userId) {
        ...
        if (orderCode) {
            ...
            try {
                ...
                return {
                    response: `❌ Có lỗi khi kiểm tra đơn hàng ${orderCode.toUpperCase()}\n\nVui lòng thử lại sau hoặc liên hệ hỗ trợ qua Telegram @edu3nova.`,
            ...
            try {
                ...
                const userOrders = await order_1.Order.find({ userId: userObjectId })
                    .populate({
                    path: 'items.courseId',
                    select: 'title image'
                })
     ...
     */
    saveToHistory(userId, message, response) {
     ...
     */
    async getAIResponse(userMessage, courses, userHistory) {
        if (!this.model) {
     ...
     */
    createPrompt(userMessage, courses, userHistory) {
        const courseList = courses
            .map((c) => `- ${c.title}: ${c.description?.substring(0, 100)}... (Giá: ${c.price?.toLocaleString('vi-VN')}đ, Level: ${c.level})`)
            .join('\n');
        ...
        const conversationHistory = userHistory
            ...
            .join('\n');
        return `
Bạn là một trợ lý AI thông minh cho nền tảng học trực tuyến EDUNOVA Learning. Bạn có thể xử lý mọi loại câu hỏi về khóa học:
...
THÔNG TIN NỀN TẢNG:
- Tên: EDUNOVA Learning - Nền tảng học trực tuyến
...
- Hỗ trợ: 24/7 qua chat, email: support@edunova.com
- Admin Telegram: @edu3nova - Liên hệ trực tiếp với admin
- Hotline: +84 978093224 - Hỗ trợ khẩn cấp
...
`;
     ...
     */
    parseAIResponse(aiText, courses) {
        try {
            ...
            if (jsonMatch) {
                ...
                if (parsed.matchedCourses && Array.isArray(parsed.matchedCourses)) {
                    parsed.matchedCourses.forEach((courseTitle) => {
     ...
     */
    simpleKeywordMatch(userMessage, courses) {
        ...
        const uniqueCourses = matchedCourses.filter((course, index, self) => index === self.findIndex((c) => c._id.toString() === course._id.toString()));
    ...
    async getAllCourses() {
        try {
            const courses = await course_1.Course.aggregate([
                { $match: { status: enums_1.CourseStatus.PUBLISHED } },
                {
                    $lookup: {
                        from: 'users',
                        localField: 'authorId',
    ...
    getFallbackResponse(userMessage) {
        ...
        if (lowerMessage.includes('hoàn tiền') || lowerMessage.includes('chính sách')) {
            return {
                response: '📋 Chính sách:\n• Hoàn tiền 30 ngày\n• Học trọn đời\n• Hỗ trợ 24/7\n• Telegram: @EdunovaLearningSupport',
        ...
        if (lowerMessage.includes('liên hệ') || lowerMessage.includes('hỗ trợ') || lowerMessage.includes('support')) {
            return {
                response: '📞 Liên hệ hỗ trợ EDUNOVA Learning:\n\n• 💬 Chat trực tiếp: Ngay tại đây\n• 📧 Email: support@edunova.com\n• 📱 Telegram: @edu3nova\n• ☎️ Hotline: +84 978093224\n\nChúng tôi hỗ trợ 24/7! 💪',
                suggestions: ['Chat ngay 💬', 'Gọi hotline ☎️', 'Telegram @edu3nova', 'Email hỗ trợ 📧'],
                intent: 'support'
            };
        }
        if (lowerMessage.includes('telegram') || lowerMessage.includes('tele')) {
            return {
                response: '📱 Kết nối với EDUNOVA Learning qua Telegram:\n\n• 👨‍💼 Admin: @edu3nova\n• 🆘 Hỗ trợ khẩn cấp: +84 978093224\n\nTelegram là cách nhanh nhất để nhận hỗ trợ! ⚡',
        ...
        if (lowerMessage.includes('chào') || lowerMessage.includes('hello')) {
            return {
                response: 'Chào bạn! 👋 Tôi là trợ lý AI của EDUNOVA Learning. Bạn cần hỗ trợ gì?\n\n💡 Tip: Liên hệ trực tiếp qua Telegram @edu3nova để được hỗ trợ nhanh nhất!',
                suggestions: ['Khóa học hot 🔥', 'Khuyến mãi 🎉', 'Liên hệ Telegram 📱', 'Xem tất cả 📚'],
                intent: 'general'
            };
        }
        return {
            response: 'Tôi là trợ lý AI của EDUNOVA Learning! 😊 Tôi có thể giúp bạn tìm khóa học, tư vấn lộ trình, hỗ trợ chính sách...\n\n📱 Liên hệ trực tiếp: @edu3nova trên Telegram hoặc gọi +84 978093224',
...
exports.CourseChatbotService = CourseChatbotService;
exports.default = new CourseChatbotService();
